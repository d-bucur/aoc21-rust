...
impl<T> Vec2d<T> where T: Clone {
    fn new(width: usize, height: usize, init: T) -> Vec2d<T> {
        Vec2d {
            nums: vec![init; width * height], width
        }
    }
    ...
    fn get_height(&self) -> usize { self.nums.len() / self.width }
}
...
impl BingoBoard {
    fn new(nums: Vec2d<u32>) -> BingoBoard {
        ...
        let mut bb = BingoBoard { nums, marked_x, marked_y, num_pos: HashMap::new(), score: 0 };
        let mut x = 0usize;
        let mut y = 0usize;
...
pub fn read_numbers(it: &mut impl Iterator<Item = String>) -> Vec<u32> {
    let numbers: Vec<u32> = it.next().unwrap()
        .split(",")
